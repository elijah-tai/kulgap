...
class Category:
    ...
    def calculate_kl_divergence(self, control):
        ...
        min_x_index = min(self.y.shape[1], self.y.shape[1]) - 1

        if control.y.shape[1] > self.y.shape[1]:
            self.kl_divergence = abs(quad(kl_integrand, self.drug_start_day, self.x[min_x_index])[0]
                                     - max(self.x) / 2)[0]
        else:
            self.kl_divergence = abs(quad(kl_integrand, self.drug_start_day, control.x[min_x_index])[0]
                                     - max(control.x) / 2)[0]
    ...
    def __repr__(self):
        ...
        return ("\nName: %s\n"
                ...
                % (self.name, str(self.drug_start_day), self.phlc_id, [r for r in self.replicates], self.kl_divergence,
                   ...
                   self.delta_log_likelihood_h0_h1))
